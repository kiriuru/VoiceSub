...
Этот README описывает текущий desktop product surface для линии `0.4.1`.
...
**Unreleased (инженерный слой, та же версия)** — [docs/CHANGELOG.md](./docs/CHANGELOG.md#unreleased): тонкий фасад `RuntimeOrchestrator` + mixins, сериализация send WebSocket/browser ASR, hardening store/панелей (без сброса caret), ротация desktop-логов, **462** collected tests.
...
- `Stream Subtitle Translator.exe` — стандартный bootstrap (payload следует `PROJECT_VERSION`, в этом дереве **0.4.1**)
...
- `backend/versioning.py` (`PROJECT_VERSION = "0.4.1"`) — single source of truth для версии приложения.
...
- версионный bundle (локально): `dist\desktop-releases\v0.4.1\` (`01-bootstrap-onefile\`, …) при publish этой линии; в старых деревьях может оставаться `v0.4.0\`.
...
Для `0.4.1`: после правок запускай `python -m unittest discover -s tests`; публичный репозиторий — tracked subset (включая `test_browser_asr_observability`). Сборка bootstrap — только локально, см. [docs/TECHNICAL_ARCHITECTURE.md](./docs/TECHNICAL_ARCHITECTURE.md) §20.
