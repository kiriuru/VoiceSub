...
`0.4.0` добавляет **наблюдаемость Browser Speech** на бэкенде (trace/replay, ingress, bounded WS, supersession preview) и **правки desktop-сборок**: новый **`Only Web.exe`**, **блокировку Parakeet** после Web Speech quick start (`desktop_profile_lock`) и **быстрый старт dashboard**. Публичные `/api` и инварианты субтитров не меняются; `config_version` — **7**.
