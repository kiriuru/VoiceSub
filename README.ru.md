...
Для `0.4.3`: после правок запускай `python -m unittest discover -s tests`; публичный репозиторий — tracked subset (включая `test_browser_asr_observability`). Сборка bootstrap — только локально, см. [docs/TECHNICAL_ARCHITECTURE.md](./docs/TECHNICAL_ARCHITECTURE.md) §20.
