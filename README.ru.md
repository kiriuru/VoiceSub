...
- Настройка темы интерфейса dashboard (светлая/тёмная) и палитры акцентных градиентов, применяемой также к окнам Browser Speech worker.
...
- Для провайдера OpenAI dashboard умеет подставлять модель из короткого рекомендуемого списка (без обращения к OpenAI API из браузера).
...
- Runtime diagnostics покрывают latency, ASR state, translation queue/provider state, Browser Speech worker connectivity, OBS Closed Captions state и local log locations.
