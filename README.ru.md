...
- **TTS-модуль** — native/Sonic dual-sink; озвучка субтитров + Twitch chat TTS (до **5 IRC-каналов** на одно OAuth, фильтры применяются на лету); громкость до **150%**
...
1. Установите **VoiceSub** из `VoiceSub_0.5.4_x64-setup.exe` (или последней сборки в папке релиза; разработчикам: `build-release-msi.bat` → `build-release.ps1`).
...
| Текст в OBS не исчезает после TTL/Stop | Обновите сборку; перезагрузите Browser Source (`overlay.js?v=20260615a`, fix idle TTL DOM clear) |
...
Текущая patch-линия **`0.5.4`** — см. [CHANGELOG (RU)](./docs/CHANGELOG.md) / [(EN)](./docs/CHANGELOG.en.md).
...
```powershell
npm install
npm run build          # dashboard + worker + TTS (включает i18n:export)
npm run i18n:export    # scripts/i18n-source → src/lib/i18n/locales/*.json
npm run i18n:bundle    # overlay locales-bundle.js (после правки scripts/i18n-source/)
cargo test --workspace
...
Версия: `voicesub-types::PROJECT_VERSION` = **`0.5.4`**.
