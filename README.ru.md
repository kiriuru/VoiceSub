...
`0.4.0` добавляет **компактный режим дашборда** с ресайзом окна в desktop, **наблюдаемость Browser Speech** на бэкенде, **блокировку Parakeet** после Web Speech quick start, **быстрый старт dashboard** в shell и второй установщик **`Only Web.exe`**. Публичные `/api` и инварианты субтитров не меняются; `config_version` — **7**.
...
- **Тесты:** **336** локально; на GitHub — suite без desktop-only тестов (`test_browser_asr_observability` и др.).
