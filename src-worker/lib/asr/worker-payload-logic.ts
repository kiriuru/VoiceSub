...
export function buildWorkerPayload(ctx: {
  state: BrowserAsrState;
  type: string;
  extra?: Record<string, unknown>;
  nowMs: number;
  visibilityState: string;
  browserSessionAgeMs: number | null;
  ...
  return {
    ...
    last_error: buildLastError(state),
    ...
    stopping_since_ms: state.stoppingSinceMs ? Math.max(0, ctx.nowMs - Number(state.stoppingSinceMs)) : null,
    ...
    mic_active_recent_ms: state.micActiveRecentMs == null ? null : Math.max(0, Number(state.micActiveRecentMs || 0)),
