...
Текущий результат для `0.4.3+` (полное dev-дерево с `desktop/`):
...
- opt-in deep diagnostics: `backend/core/diagnostic_flags.py` + `SST_DEEP_DIAGNOSTICS` / `SST_TRACE_*` — JSONL-traces и `runtime_lifecycle.*` события по умолчанию выключены; backbone logs (`backend.log`, `runtime-events.log`, `session-latest.jsonl`) остаются как в 0.4.1 baseline.
- subtitle renderer DOM lifecycle (0.4.3): WeakRef в persisted render state, orphan surface scrub перед slow-path rebuild, `disposeRenderContainer` при unmount dashboard preview.
