# SST Desktop 0.4.1 — Технический документ

Актуально для линии кода, где `backend/versioning.py` содержит `PROJECT_VERSION = "0.4.1"`.
...
- `backend/versioning.py::PROJECT_VERSION = "0.4.1"` — single source of truth.
...
Текущий результат для `0.4.1+` (полное dev-дерево с `desktop/`):
...
- packaged release folders (после publish): `dist/desktop-releases/v0.4.1/` (`01-bootstrap-onefile/`, `01-bootstrap-web-only-onefile/`, `02-managed-app-onefolder/`, `03-installers-both/`)
...
## 21. Продуктовые инварианты, сохранённые в 0.4.1
...
- **translation lifecycle** (§12.3): completed translation block не снимается первым partial новой фразы;
...
**Post-0.4.1 engineering hardening (без смены публичных API-контрактов):**
...
- regression suite расширен до **462** collected tests (§20).
