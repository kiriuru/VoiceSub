...
Единая история изменений desktop-линии: **VoiceSub** `0.5.5` (текущая линия), **VoiceSub** `0.5.4`, **VoiceSub** `0.5.3`, **VoiceSub** `0.5.2`, **VoiceSub** `0.5.1`, **VoiceSub** `0.5.0` (первый релиз новой линии, baseline — SST `0.4.4`) и **SST Desktop** `0.4.4` и ниже.
...
Patch release. `PROJECT_VERSION` — **0.5.5**; `config_version` **8** (без изменений). Фокус: дальнейшее снижение нагрузки Tauri IPC и CPU на hot path субтитров/ASR — коалесинг `overlay_update` IPC для dashboard (OBS WebSocket без изменений), развязка ingest и fanout, lock-free WS event sequencing, метрики bus lag, безопасный debounce lag-resync; **hotfix:** автопереподключение Web Speech при `audio-capture`. **Контракты без изменений:** OBS `/ws/events` по-прежнему получает каждый `overlay_update`; lifecycle субтитров, shape overlay payload, протокол browser worker `/ws/asr_worker`.

### Runtime — Tauri IPC pump (CPU / WebView2)

- **`src-tauri/src/ipc_pump.rs`** (новый) — отдельный bus→IPC pump: **trailing-edge коалесинг** `overlay_update` только для **main dashboard** (default 90 ms, env `VOICESUB_OVERLAY_IPC_MIN_INTERVAL_MS`; `0` = выкл.). OBS overlay и клиенты `/ws/events` получают каждый кадр.
- **`runtime_update` / `translation_update`** немедленно сбрасывают pending overlay.
- При `broadcast::RecvError::Lagged`: метрики, **debounce** полного snapshot resync (200 ms), resync в **фоновой задаче** — pump продолжает обрабатывать события и overlay-таймеры.
- Flush pending overlay при закрытии bus.

### Runtime — ingest vs fanout

- **`transcript_controller.rs`** — subtitle lifecycle **до** WS/IPC fanout; partial `transcript_update` — **async** (`tokio::spawn`), ingest не блокируется на enrich/broadcast.
- Final по-прежнему await `transcript_update`; порядок для `submit_final` сохранён через await `handle_transcript`.

### Runtime — WS enrich + diagnostics

- **`voicesub-ws/event_sequence.rs`** — `global_sequence` через `AtomicU64`; hot-path enrich без внешнего `Mutex`.
- **`RuntimeMetricsCollector`** — `event_bus_consumer_lagged_total`, `event_bus_consumer_lagged_messages_skipped`, `overlay_ipc_coalesced_suppressed` в metrics `/api/runtime/status`.

### Dashboard

- **`App.svelte`** — пропуск 4 s HTTP poll при активном Tauri IPC; safety-net poll 30 s при подключённом IPC.

### Browser worker — auto-retry `audio-capture`

- **`src-worker/lib/asr/`** — ошибка Web Speech **`audio-capture`** больше не останавливает распознавание навсегда: экспоненциальный backoff (как у `network`), сброс монитора микрофона, повторный `getUserMedia` и автоматический рестарт сессии. Терминальными остаются только `not-allowed` / `service-not-allowed`.

### Документация

- **`docs/TECHNICAL_ARCHITECTURE.en.md`** / **`docs/TECHNICAL_ARCHITECTURE.md`** — IPC pump, env overlay IPC coalescing, порядок ingest.

### Env (новые / затронутые в 0.5.5)
...
| Сборка | `build-release-msi.bat` → `build-release.ps1` → `F:\AI\VoiceSub - release\v0.5.5\` |

### Миграция

- `config_version` **не менялся** — конфиги 0.5.4 загружаются как есть.
- Live preview dashboard может обновлять overlay IPC с задержкой до ~90 ms при быстрых partial; тайминг OBS overlay **без изменений**.

## 0.5.4

Patch release. `PROJECT_VERSION` — **0.5.4**; `config_version` **8** (без изменений). Фокус: снижение CPU/WS/IPC нагрузки на hot path субтитров и ASR, per-window маршрутизация Tauri `runtime-event`, снижение системного влияния Chrome-воркера, устойчивость TTS/Twitch к сетевым сбоям, надёжность Rust TTS pipeline (prefetch, config I/O, порядок audio-chunks), **полная чистка TTS-модуля** (удаление JS queue pump и deprecated IPC), исправление лога Twitch-чата в модуле TTS, диагностика ingest latency при полном логировании, исправления HTTP/WS fanout. **WebSocket:** live субтитры — только `overlay_update`; ASR — только `transcript_update` (удалён неиспользуемый дубль `transcript_segment_event`). `subtitle_payload_update` — **только Tauri IPC snapshot** (не публикуется на `/ws/events`; WS replay on connect — `runtime_update` + `overlay_update`). Payload overlay/ASR **не менялись**. Browser worker `/ws/asr_worker` **без изменений** (протокол); изменён только класс приоритета процесса и reap осиротевшего PID при старте.

### Runtime — ASR WS fanout (CPU)

- **`transcript_controller.rs`** — убран дублирующий broadcast `transcript_segment_event` (тот же payload, что `transcript_update`); в tree нет потребителей (dashboard, overlay, TTS — только `transcript_update`).
- **Эффект:** ~−50% WS broadcast на ASR partial/final path.

### Runtime — subtitle WS fanout (CPU / OBS)

- **`service.rs`** — убран дублирующий live broadcast `subtitle_payload_update` на каждый publish; OBS overlay и dashboard preview получают live кадры через `overlay_update` (`OverlayBroadcaster` + dedupe).
- **`HttpState::flush_overlay_presentations_to_clients`** — flush только `overlay_update` (без второго идентичного сообщения).
- **`runtime_state_snapshot`** — поле `subtitle` читается из `last_subtitle_payload` (без `MutexGuard` через `.await` в Tauri IPC).
- **Эффект:** ~−50% WS broadcast/мин на субтитровом пути при непрерывной речи (подтверждено логами); lifecycle/presentation **без изменений**.

### Runtime — browser ingest hot path

- **`browser_speech_source.rs`** — sync `accept_update` + async `process_ingest_work`: ingest mutex **не** удерживается на время `handle_event` / WS / subtitle actor.
- Убран полный `config.toml` clone на каждый partial; `source_lang` читается точечно из snapshot.
- **`transcript_controller.rs`** — `source_text_replacement` парсится из секции без clone всего config.

### Runtime — Tauri IPC fanout & system impact

- **`src-tauri/src/event_routing.rs`** (новый) + **`lib.rs`** — `runtime-event` маршрутизируется через `emit_to(label, …)`: главное окно получает все типы, окно TTS — только `twitch_*` / `runtime_update` / `runtime_status` / `ui_config_sync`. Высокочастотный `transcript_update` / `overlay_update` больше не флудит IPC-канал TTS-вебвью. Payload передаётся по ссылке (без deep-clone на событие).
- **`lib.rs`** — при `broadcast::RecvError::Lagged` пул IPC ре-синхронизирует дашборд снапшотом (`snapshot_to_envelopes`) вместо тихого пропуска. Снапшот включает `twitch_connection` (status), поэтому connection UI окна TTS восстанавливается после lag; chat-сообщения эфемерны и не реплеятся.
- **`transcript_controller.rs`** — leading-edge коалесинг partial **`transcript_update` only** (по умолчанию 90 ms, env `VOICESUB_TRANSCRIPT_PARTIAL_MIN_INTERVAL_MS`; новая фраза/`sequence` и все final — без задержки). Subtitle lifecycle, `overlay_update`, OBS overlay и dashboard preview **видят каждый partial**; коалесится только избыточный канал сырого ASR (Overview «Partial», не live render).
- **`overlay_broadcaster.rs`** — `dedupe_signature` (deep-clone + сортировка ключей + serialize) вычисляется только для стабильных состояний; на `partial_only` / `completed_with_partial` пропускается; partial между двумя identical `completed_only` сбрасывает dedupe-якорь.
- **`voicesub-ws/events.rs`** — `enqueue_to_client` принимает `message_type` от вызывающего; убран повторный парсинг JSON на каждого WS-клиента.
- **`service.rs`** — `SubtitlePayloadForwarder`: TTS-listener вызывается на отдельном упорядоченном потоке (`voicesub-subtitle-payload-forward`), а не внутри publish-цикла subtitle actor. При переполнении очереди дропается старейший **не-speakable** кадр (partial-only / active), speakable (`completed_*`) сохраняются; паника listener изолируется (`catch_unwind`) — поток форвардера не падает.
- **Тесты:** `event_routing` (per-window routing + snapshot envelopes), `transcript_controller` (`partial_throttle_*`), `overlay_broadcaster` (`partial_frame_clears_completed_dedupe_anchor`).

### Browser worker — приоритет и осиротевшие процессы

- **`launcher.rs`** — Chrome-воркер запускается с `ABOVE_NORMAL_PRIORITY_CLASS` вместо `HIGH_PRIORITY_CLASS`: ASR остаётся отзывчивым (анти-throttling + EcoQoS opt-out сохранены), но не вытесняет с CPU другие программы. Fallback на normal при `ERROR_ACCESS_DENIED`.
- **`orphan_guard.rs`** (новый) — PID воркера сохраняется в `user-data/browser-worker.pid` при launch (`http/runtime.rs` **и** Tauri IPC `launch_browser_worker` → `service.rs`), удаляется при graceful stop; `RuntimeService::start` убивает осиротевший воркер прошлой *аварийной* сессии, только если PID всё ещё принадлежит процессу Chromium (`chrome.exe`/`msedge.exe`) — защита от переиспользования PID.
- **Тесты:** `orphan_guard` (PID roundtrip, image guard, tasklist CSV parse).

### Browser worker — overlap ASR handoff

- **`overlap-logic.ts` / `recognition-handlers.ts` / `session-manager.ts`** — pre-start buddy-слота вызывается на границе сегмента и при **подавленном** (duplicate / late-forced) final, а не только при успешном — идемпотентно (guard `recognitionOverlapPrestarted`), не на teardown-пути. Восстанавливает overlap, если предыдущий pre-start упал.
- **`overlap-logic.ts`** — retry pre-start (`scheduleBuddyPrestartRetry`) хранит handle таймера в state, привязан к `recognitionGenerationId` и отменяется в `clearAllTimersInternal` / при новой паре слотов — таймер больше не может вызвать `buddyRec.start()` на завершённой сессии.
- **`overlap-logic.ts` / watchdog** — `shouldRestartStalledOverlapHandoff`: если warming-buddy промоутнут в active, но его `onstart` не сработал за `buddyGhostActiveMicMs`, watchdog делает быстрый rearm вместо ожидания visible-idle (~30s).
- **Поведение:** pre-start buddy — только по committed-сегменту; time/speech-based ранний prestart не используется (возможна чуть большая задержка на первой фразе — by design).
- **`recognition-handlers.ts`** — убран недостижимый duplicate guard в `case "aborted"` (overlap уже отсекается в `applyRecognitionError`).
- **Тесты:** `overlap-logic.test.ts` (retry timer cleanup, stalled handoff, suppressed-final prestart).

### Browser worker — long-segment flush (post-monologue fragmentation)

- **`long-segment-flush-logic.ts`** (новый) — после committed final (natural или forced), если пик partial/final сегмента ≥ **200** символов, worker сбрасывает «забитый» буфер Web Speech `results`: в `native_continuous` — `recognition.flush` (`long_segment_flush`, delay как `session_cycle` ~150 ms); в overlap (`continuous=false`) — `stop()` только активного слота **после** `preStartNextOverlapInstance` для race-safe handoff на buddy.
- **`recognition-handlers.ts` / `session-manager.ts`** — учёт `currentSegmentPeakPartialChars` на interim; flush не на partial и не при pending restart/stopping.
- **`recognition-lifecycle.ts`** — `requestRecognitionFlush`; `transitionToStopping` для `long_segment_flush` не вызывает `forceFinalizeOnInterruption` (сегмент уже committed).
- **Симптом (до фикса):** после длинного монолога (300–400+ символов в одном final) следующая речь стабильно рвалась на короткие final (десятки символов) — подтверждено `pipeline-trace.jsonl` (`asr_ingest_final_published`).
- **Инварианты без изменений:** overlap prestart только на final, force-finalize idle 1600 ms, dedup partial/final, subtitle lifecycle.
- **Тесты:** `long-segment-flush-logic.test.ts`.

### Diagnostics — pipeline ingest latency

- **`trace.rs`** + **`transcript_controller`** — поле `ingest_latency_ms` в `pipeline-trace.jsonl` при `logging.full_enabled` (или deep diagnostics); метрики runtime collector.
- **Тест:** `asr_ingest_published_includes_latency_when_provided`.

### Формат timestamp (diagnostics / subtitle lifecycle) — ⚠ изменение

- **`voicesub-types/time.rs`** (новый) — RFC 3339 UTC helper (`utc_now_rfc3339`, `epoch_secs_to_rfc3339`) без внешних crate.
- **`voicesub-logging` (`session.rs`, `jsonl_trace.rs`)** и **`voicesub-subtitle/lifecycle.rs`** — поля `timestamp_utc`, `finalized_at_utc`, `completed_expires_at_utc` теперь **RFC 3339 строка** (`2026-06-21T07:01:00Z`) вместо численных epoch-секунд (`"1781806869"`). Структура payload (ключи) не изменилась; изменился только формат значения.
- **Влияние:** overlay/dashboard render эти поля не парсит как число — рендер субтитров не затронут. Внешние скрипты/диагностика, ожидавшие epoch-секунды, должны принимать обе формы. `tools/_analyze_tts_session.py` обновлён (`ts()` принимает epoch и RFC 3339).

### TTS — network resilience + speech settings

- **`upstream_retry.rs`** — общий retry helper (3 попытки, transport + HTTP 5xx/429/408).
- **`google_fetch.rs`** — retries, connect/read timeouts на reqwest client.
- **`python_runtime.rs`** — тот же retry helper для embedded fetch.
- **`http/tts_proxy.rs`** — делегирует в `fetch_google_tts_browser` (без дублирующего HTTP client).
- **`config.rs`** — `normalize_speech_settings()` clamp `min_chars` 1–32, `max_queue_items` 1–64.
- **Тесты:** `subtitle_speech.rs`, `subtitle_speech_service.rs` (`min_chars`, saved settings).

### TTS — pipeline, prefetch & config I/O

- **`google_fetch.rs`** — `assemble_ordered_chunks`: длинный TTS-текст собирается в исходном порядке после параллельного fetch (`JoinSet` отдавал чанки по завершению, не по индексу). Конкурентность fetch ограничена семафором (`GOOGLE_TTS_PREFETCH_MAX_CONCURRENCY = 3`) — не бурстит много параллельных запросов к Google TTS (429/503). Тесты: `assemble_ordered_chunks_*`.
- **`channel_orchestrator.rs`** — guard от параллельного duplicate prefetch (`in_flight`); ожидание prefetch через `Notify` вместо busy-poll; Twitch-канал использует `effective_speech_rate` / `effective_speech_volume`; `check_stuck` / `clear` / `set_enabled(false)` симметрично отменяют `completion_waiter`.
- **`config.rs`** — in-memory cache для hot-path reads (инвалидация по mtime файла — внешние правки `config.toml` подхватываются без рестарта); atomic save (temp + rename); backup повреждённого `config.toml` вместо silent overwrite defaults. Тесты: legacy migrate, corrupt backup, temp cleanup, mtime invalidation.
- **`ipc.rs`** — `speech_queue_item_id`: monotonic counter внутри millisecond bucket (коллизии id при burst enqueue).
- **`service.rs`** — adaptive queue cap читает `max_queue_items` из normalized speech settings, не hardcoded `8`.

### TTS — subtitle speech planner

- **`subtitle_speech.rs`** — планирование озвучки для `completed_with_partial` (раньше только `completed_only`); `sequence` из `completed_sequence`; partial source line пропускается, пока фраза не finalized. Тесты: `subtitle_speech_service.rs`.

### TTS module — Twitch chat log

- **`twitch-chat-log.ts`** (новый) — dedupe входящих chat events по Twitch `id` / `event_sequence` перед prepend в лог.
- **`TwitchPanel.svelte`** — уникальный `logKey` для keyed `{#each}` при пустом/повторяющемся IRC `id` (иначе Svelte замирает после первой строки).
- **`twitch-chat-log.ts`** — dedupe fallback без стабильного id/seq/`created_at_ms` использует уникальный per-row ключ (`logKey`) вместо `user|channel|text` — два разных сообщения с одинаковым текстом больше не схлопываются.
- **`runtime-events.ts`** — новый `runtime-event` listener подключается **до** отписки предыдущего: транзиентный сбой `listen` не оставляет окно без IPC-канала.
- **`App.svelte`** — generation guard в `connectRuntimeEvents` (отмена stale async connect); авто-reconnect IPC-канала в 3s runtime-таймере, если он отвалился.
- **Тесты:** `twitch-chat-log.test.ts` (fallback dedupe), `runtime-events.test.ts` (twitch_connection replay).

### TTS — громкость 150% и числовые подписи (Speech / Twitch)

- **`voicesub-audio/playback.rs`** — `SPEECH_VOLUME_MAX = 1.5`, `clamp_speech_volume()`; native playback через `rodio` `amplify()` (0–150%).
- **`voicesub-tts/config.rs`** — `normalize_tts_config` clamp root `speech_volume` и twitch override (`>= 0`); **`service.rs`** — `update_voice_settings`.
- **`src-tts`** — слайдеры громкости `max="1.5"` (`App.svelte`, `TwitchPanel.svelte`); `speech-playback-policy.ts` (`PLAYBACK_VOLUME_MAX`, `clampSpeechVolume`); `playback-format.ts` (`formatSpeechVolume` → `150%`).
- **Twitch advanced** — override скорости/громкости с live-числами как на вкладке Speech (`1.25×`, `85%`); слайдер rate — `oninput` + `onchange` (число обновляется при перетаскивании).
- **Тесты:** `clamp_speech_volume_allows_one_hundred_fifty_percent`, `normalize_clamps_speech_volume_to_one_hundred_fifty_percent`, `playback-format.test.ts`, `apply_speech_volume_boosts_quiet_pcm_above_unity`.
- **Fix (playback):** native path — decode → `apply_speech_volume_to_pcm` (f32) вместо rodio `i16` `amplify()`; **>100%** — compression + makeup gain + brick-wall limit (не линейный `×1.5` на уже клипнутом TTS); browser — тот же алгоритм на `AudioBuffer`; UI — debounced save при перетаскивании слайдера.

### TTS module — cleanup JS queue & deprecated IPC

- **Sample test** — кнопка Speak test в `/tts` больше не использует JS `SpeechEngine`; IPC **`tts_speak_sample`** → `TtsSpeechPipeline::enqueue_speech_test` → `ChannelOrchestrator` (тот же hot path, что subtitle/twitch). Activity log — event `tts-speech-activity`; статус — `playback-finished` на канале `speech`. Sample test **не требует** `runtime_active` (в отличие от subtitle path).
- **Удалены frontend-модули:** `speech-engine.ts` (~965 строк), `browser-audio-output.ts`, `twitch-replacements.ts`, `theme.ts`, `source-text-replacement.ts`.
- **`google-tts.ts`** — оставлены warmup, URL builder и chunking; удалены WebAudio/HTMLAudio playback (`playGoogleTts`, prefetch-to-speaker path ~200 строк).
- **`audio-player.ts`** — только helpers режима (`isNativePlaybackMode` / `isSonicPlaybackMode`); `NativeAudioPlayer` и JS pump удалены.
- **`speech-playback-policy.ts`** — только clamp/format helpers; queue-depth rate boost (дубль Rust `playback_policy.rs`) удалён.
- **`tts-ipc.ts`** — удалены `planSubtitleSpeech`, `openTtsWindow`, `syncSourceTextReplacement`, `channelEnqueue` / `channelBeginNext` / `channelFinish` / `channelSnapshot`; добавлен `speakSample`.
- **`translation-lines.ts`** — удалён legacy test-target API (`buildSpeakTargets`, localStorage target id).
- **`App.svelte`** — без `SpeechEngine`; `clearSpeechChannels` через `tts_channel_clear`; убран `applyDashboardConfigPayload`.
- **`TwitchPanel.svelte`** — неиспользуемый prop `onConnectionChange`.
- **Rust `voicesub-tts`** — `enqueue_speech_test`, `wake_after_enqueue`; удалены мёртвые методы `TtsModuleService` (OAuth wrappers, `save_config`, `list_output_devices`, `queue_clear*`, …), `SpeechQueueState::Paused` / `pause` / `resume`; `channel_queue` использует `voicesub_audio::CHANNEL_*`; сужены re-exports в `lib.rs`.
- **⚠ Breaking (Tauri IPC):** удалены **`tts_enqueue`**, **`tts_plan_subtitle_speech`**, **`tts_channel_enqueue`**, **`tts_channel_begin_next`**, **`tts_channel_finish`**, **`tts_channel_snapshot`**, **`tts_sync_source_text_replacement`**. Subtitle speech планируется только в Rust (`speech_pipeline::handle_subtitle_payload`). Очистка очереди speech — **`tts_channel_clear`**; recovery — **`tts_channel_force_idle`**.
- **Тесты:** `speech_pipeline::speech_test_*`, `google-tts.test.ts`, `speech-playback-policy.test.ts`; `cargo test --workspace`, `npm run test:frontend`.

### Twitch — фильтры чата (mentions, цифры, ссылки)

- **`lang.rs`** — `normalize_twitch_mentions()` для TTS (`@nick` → `nick`, текст сохраняется); clean/detection path — полный `strip_twitch_mentions`.
- **`pipeline.rs`** — TTS `speak_text` и UI `clean_text` из разных веток; `has_meaningful_linguistic_content(text, strip_links)` — link strip/reject только при `strip_links=true`.
- **`emoji.rs`** — `strip_invisible_chat_characters` (U+034F, U+3164, `\p{Cf}`, …) до фильтров; **`lang.rs`** — `is_digit_group_token` (`500&100`, `500$100` и т.п.).
- **`symbols.rs`** — `&`/`$` между цифрами → пробел; URL-параметры не ломаются.
- **`links.rs`** — маркеры `github.com/` и др.; **`strip_leading_speaker_label`** — не трактует `https:` как метку спикера.
- **Тесты:** `voicesub-twitch` (mentions, digit groups, link-only при `strip_links=false`).

### Twitch — IRC reconnect hardening

- **`irc.rs`** — TLS EOF / `close_notify` → `Disconnected` (retry), не fatal `Error`; jitter на reconnect backoff; `TCP_NODELAY`; pong failures → disconnect.
- **`service.rs`** — UI cleanup если reconnect loop завершился в `connecting`.
- **`error.rs`** — `is_retryable()` для auth vs network.

### Runtime — HTTP/WS path fixes

- **`voicesub-ws/publisher.rs`** — sync-fallback (`broadcast_*_now` вне Tokio runtime): убран второй `RuntimeEventBus::publish` в worker-потоке; Tauri `runtime-event` больше не получает дубликат overlay/translation кадра. Тест: `broadcast_now_publishes_event_bus_exactly_once_outside_tokio`.
- **`http/ui_sync.rs`** — `POST /api/ui/sync` через `ws_publisher.broadcast_channel` вместо прямого `EventsHub::broadcast`: `ui_config_sync` получает `event_sequence`/`created_at_ms` enrichment и попадает в `RuntimeEventBus` (Tauri IPC), как остальные каналы.
- **`voicesub-ws/events.rs`** — из WS replay on connect убран мёртвый `subtitle_payload_update` (тип больше не публикуется на hub); `replay_last` — один `clients.read()` на вызов, ошибка сериализации не шлёт пустой Text-кадр клиенту. Тесты: `subtitle_payload_update_not_replayable`, `replay_last_absent_type_returns_true`.
- **`service.rs`** — `SessionLogManager` создаётся один раз в `build()` и переиспользуется в `http_state()` (не пересоздаётся при каждом вызове).
- **`http/openai.rs`** — `usable_models` задокументирован как alias `list_models`; поля `OpenAiModelsRequest` помечены `#[allow(dead_code)]` (контракт API, статический список моделей).

### Config — deprecated lifecycle timing keys

- **`pause_to_finalize_ms`** / **`finalization_hold_ms`** и **`hard_max_phrase_ms`** / **`max_segment_ms`** помечены **deprecated** в `config-normalize.ts`, `voicesub-config/normalize.rs`, `data/config.schema.json` (`"deprecated": true` + description), `types.ts`, `lifecycle.rs`.
- Ключи **сохраняются** при normalize/load для старых конфигов и trace parity, но **не влияют на runtime** (FSM субтитров и воркер их не читают).
- **Актуальная** настройка forced-final idle — `asr.browser.force_finalization_timeout_ms` (UI окна Web Speech worker).
- **`data/config.example.json`** — deprecated ключи убраны из примера (normalize подставит при импорте).

### Dashboard — расширенные настройки Web Speech

- **Новые defaults** (live-stream tuning, synced `webspeech-advanced-defaults.ts` → `config-normalize.ts`, `defaults.rs`, `worker-defaults.ts`, worker session fallbacks, `config.schema.json`):
...
| остальные advanced | — | без изменений |

- **`WebSpeechAdvancedSettings.svelte`** — у каждого поля кнопка **`!`** (`FieldHelpButton.svelte`) с локализованным описанием влияния (en, ru, ja, ko, zh); popover по клику + `title` при hover.
- **i18n:** `settings.webspeech.advanced.<field>.help`, `settings.webspeech.advanced.help_trigger` в `scripts/export-i18n.mjs`.

### Dashboard / frontend — UI cleanup (консервативная чистка)

- **Форматирование** — нормализованы двойные пустые строки в `SettingsPanel.svelte`, `SubtitlesPanel.svelte`, `TranslationPanel.svelte`, `DashboardPanels.svelte` (без изменения логики).
- **`SettingsPanel.svelte`** — убран дубль `settings.fonts.hint`; секция шрифтов: `settings.fonts.eyebrow` / `settings.fonts.title`; inline `margin-top` → `.section-heading--spaced` (`global.css`).
- **`WebSpeechAdvancedSettings.svelte`** — те же spacing-классы вместо inline `style`.
- **`StylePanel.svelte`** — один заголовок на секцию base/slots (без дублирующего eyebrow); убрана лишняя подсказка `settings.fonts.hint` внизу; кнопка удаления пресета → `style.custom_preset.delete`.
- **`ReplacementPanel.svelte`** — единый паттерн `$: tr = …`.
- **`ToolsPanel.svelte`** — убран лишний `refreshLists()` после export diagnostics; под runtime-блоком — `tools.runtime.note`.
- **`SubtitlesPanel.svelte`** — удалён локальный `.checkbox-row` (используется global); текст `subtitles.display_order` приведён к UI reorder (up/down), не «comma-separated ids».
- **`MoreSettingsHub.svelte`**, **`SubtitlesSettingsHub.svelte`** — `.muted--flush` вместо inline `margin: 0`.
- **i18n prune** — ~**27** orphan-ключей × 5 локалей из dashboard bundle (`worker.description`, `worker.microphone.*`, `worker.metric.*` кроме `websocket`, `header.*`, `document.title.dashboard`, неиспользуемые `tools.source_replacement.*`); каталог **736** ключей × 5 локалей (было ~763). Скрипт: расширен `scripts/prune-asr-orphan-i18n.mjs`.
- **i18n (новые ключи):** `settings.fonts.title`, `settings.fonts.eyebrow`, `style.custom_preset.delete`; **`help.tools.body`** — word replacement и advanced ASR указаны в More → Word Replace и Settings (не в Tools & Data).
- **TTS locales** — удалены 8 orphan-ключей из каждого `tts-{en,ru,ja,ko,zh}.json` (`browse_audio`, `playback_mode.browser`, `speaking_twitch`, …).
- **`src/lib/i18n/index.ts`** — удалён неиспользуемый export `tStore`; **`ui-config-sync.ts`** — удалён `mergeUiConfigPatch`; **`style-presets.ts`** — `presetDescription` private.
- **Worker** — fallback `appVersion` в `worker-ui.svelte.ts` синхронизирован с **0.5.4**.
- **Overlay** — из `overlay.html` убран неиспользуемый script `dynamic-locales.js` (ключи overlay уже в `locales-bundle.js`); copy-step убран из `scripts/build-locale-bundle.mjs`; мёртвый буфер `debugEntries` в `overlay.js`.
- **Контракты без изменений:** config payload, subtitle/overlay lifecycle, HTTP/WS; deprecated `pause_to_finalize_ms` / `hard_max_phrase_ms` sync не трогали (кроме удаления deprecated **TTS** IPC — см. § TTS module cleanup).

### Документация

- **`docs/TECHNICAL_ARCHITECTURE.en.md`** / **`docs/TECHNICAL_ARCHITECTURE.md`** — полная сверка с кодом 0.5.4: Material 3 navigation, `POST /api/ui/sync`, Tauri IPC (`tts_speak_sample`, удаление JS queue IPC), TTS pipeline (`upstream_retry`, twitch-chat-log), **громкость 0–150%**, Twitch chat filters (mentions, `strip_links`, invisible chars), env vars, убран несуществующий `xtask`; **§12** advanced Web Speech defaults + field help UI; **§14** deprecated `pause_to_finalize_ms` / `hard_max_phrase_ms`.
- **`docs/WIKI.en.md`** / **`docs/WIKI.ru.md`** — расширенные настройки Web Speech; TTS planner/sample test через Rust orchestrator.

### Env (новые / затронутые в 0.5.4)

| Variable | Назначение |
| --- | --- |
| `VOICESUB_TRANSCRIPT_PARTIAL_MIN_INTERVAL_MS` | Минимальный интервал partial `transcript_update` (default **90**; `0` = без коалесинга). Не влияет на `overlay_update`. |
| `VOICESUB_BROWSER_AFFINITY` / `VOICESUB_BROWSER_AFFINITY_MASK` / `VOICESUB_BROWSER_AFFINITY_EXCLUDE_LOW` | CPU affinity воркера (без изменений; документированы в arch §12). |
...
| Сборка | `build-release-msi.bat` → `build-release.ps1` → `F:\AI\VoiceSub - release\v0.5.4\` |
...
- **`run_session_with_reconnect`** (`voicesub-twitch/irc.rs`) — после разрыва IRC/TLS или закрытия потока автоматически переподключается с exponential backoff **1→30 s**; backoff сбрасывается после успешного JOIN.
- **Non-retryable** — ошибки OAuth/nick и `InvalidSettings` останавливают цикл со статусом `error`; ручной **Disconnect** — через `stop_rx` + abort задачи.
- **UI** — при переподключении статус снова `connecting` (кнопка Disconnect доступна); WS payload `twitch_connection_update` без изменений.
- **Тесты:** `TwitchError::is_retryable()` (auth vs network).
...
- **i18n:** удалены устаревшие ключи SST (legacy local ASR UI, RNNoise tuning UI, removed diagnostics keys) из `scripts/i18n-source/`; каталоги **876** ключей × 5 локалей.
...
- **Sample test** в TTS UI остаётся на `SpeechEngine` (ручной preview); live speech/twitch — только Rust pipeline. *(Исправлено в **0.5.4**: `tts_speak_sample` → Rust orchestrator.)*
...
### ASR worker (Web Speech) — overlap и browser-trace

- **`overlap-logic.ts`** — dual-slot overlap при `continuous=false`: buddy prestart **только после final** (SST parity); `handleInactiveOverlapBuddyEnded` (buddy `onend` без global restart); `shouldIgnoreOverlapBuddyError` для ожидаемых buddy `no-speech`/`aborted`/`network`; ghost buddy recovery после sustained idle; handoff сбрасывает stale `pendingRestartReason`.
- **Overlap telemetry** — `buildOverlapTelemetrySnapshot` в каждый worker status/heartbeat (`overlap_mode_desired`, `overlap_active`, slot listening/prestart flags); Rust `GatewayDiagnostics` + overlap-поля в `browser_worker_status` / heartbeat; события `browser_overlap_buddy_ended` / `browser_overlap_buddy_error` / `browser_overlap_buddy_ghost_recovered` в `browser-trace.jsonl`.
- **Hotfix overlap prestart** — убраны speech/partial и time-based buddy prestart (вызывали одновременное прослушивание обоих слотов → `aborted` + ping-pong handoff ~1–2 s); expected handoff `aborted` больше не эмитит `browser_onerror`.
- **`recognition-handlers.ts`** — согласованы overlap guards, `overlapResultAllowed` как в SST.
...
- **Новое:** `src/lib/runtime-events.test.ts`, `src-tts/lib/speech-activity-log.test.ts`, `src-worker/lib/asr/overlap-logic.test.ts`, `crates/voicesub-browser/tests/browser_asr_gateway.rs` (overlap telemetry mapping), `chrome_launch_contract.rs`, расширен `voicesub-obs` send integration suite, `voicesub-ws` event bus tests.
...
- Splash-профили, `Stream Subtitle Translator Only Web.exe`, `desktop_profile_lock` для unlock legacy local ASR.
...
> **Frozen line.** SST `0.4.4` — read-only reference (`F:\AI\stream-sub-translator`). Активная разработка — VoiceSub `0.5.5`.
...
- **Layout:** боковый блок `tools.notes.*` удалён — пояснения только через `?` у каждого поля; legacy local ASR extras участвуют в сетке через `display: contents`; в **compact** — одна колонка (`compact-layout.css`).
...
- **SST local ASR model installer**: добавлен потокобезопасный кеш результата integrity-check локальной модели по ключу `(file_path, mtime_ns, size, expected_sha)`. SHA-256 многогигабайтного `.nemo` теперь считается один раз за жизнь процесса вместо каждого вызова `/api/runtime/status` и `/api/health`. На свежей установке с `sha256` в `manifest.json` idle-latency status падает с 3–10 с до миллисекунд.
- Публичная API инвалидации integrity-cache; вызывается после записи манифеста, чтобы закрыть гонку «`shutil.move` → manifest write».
- Регрессии: `tests/test_local_asr_model_installer_manifest.py` — 8 тестов, включая прямой regression `test_integrity_state_caches_sha256_result`.
...
### Dashboard UI: compact layout и legacy local ASR settings
...
- Compact-mode hide-правила декоративных `.eyebrow`, `<p class="muted">` под заголовками и stand-alone `<p class="muted" data-i18n>` теперь **исключают** technical-панели `recognition`, `tuning`, `asr_advanced` через `:not([data-tab-panel="..."])`. Технические подсказки и notes на legacy local ASR страницах остаются видимыми в compact-режиме (раньше агрессивно скрывались).
...
- **`frontend/js/panels/asr/asr-panel-render.js`** — local ASR tuning controls (latency preset, streaming decode toggle, `partial_emit_mode`, `partial_min_new_words`) теперь видны всегда, кроме случая `desktop_profile_lock="browser_speech"` (Web-Speech-only install). Раньше эти контролы скрывались при текущем `asr.mode === "browser_google"`, и пользователь не мог настроить local ASR до переключения mode. Регрессия: `tests/test_frontend_architecture.py::test_local_asr_tuning_controls_visible_outside_browser_speech_lock`.
...
### Runtime orchestrator и legacy local ASR
...
- `local_asr_provider`: кэш результата `nvidia-smi` (не блокировать event loop на каждом diagnostics/status).
...
- **Legacy local ASR realtime:** streaming decode, `word_growth` partial policy, delta ASR queue, overlay partial dedup bypass; `LocalAsrPipeline` и `local_asr_realtime_settings`.
...
- **Продукт:** только low-latency preset локальной модели в UI; миграция старых quality preset в low latency.
- **Диагностика:** расширенный `AsrDiagnostics` (preset / streaming / emit mode / min words).
- **Документация:** обновлён `docs/TECHNICAL_ARCHITECTURE.md` (local ASR pipeline 0.4.1).
...
- **Web Speech quick start:** `asr.desktop_profile_lock` в схеме config и после save/load; Recognition без legacy local ASR до запуска с GPU/CPU (`desktop-profile-lock.js`, нормализаторы, packaged launcher).
...
- `docs/TECHNICAL_ARCHITECTURE.md`: актуализация под `0.3.2`, `config_version` 7, поток `source_text_replacement`, **расширенный раздел про legacy local ASR** (VAD, очередь сегментов, RNNoise, два провайдера quality vs low-latency, связь с `subtitle_lifecycle`).
...
- локальный AI-путь и `browser_google` не удалены; legacy local ASR остаётся доступным;
- неподдерживаемые эксперименты backend ASR убраны с активной продуктовой поверхности; остаются только legacy local ASR и режимы browser worker.
...
- migration v3 переводит legacy realtime preset на low-latency preset;
- устаревшие настройки исторического backend ASR при нормализации возвращаются к поддерживаемым дефолтам local ASR.
...
- текущая поверхность ASR ограничена legacy local ASR и двумя режимами browser worker;
