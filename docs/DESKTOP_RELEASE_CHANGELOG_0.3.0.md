...
Сводка изменений относительно `0.2.9.2`.

Изменения после выхода релиза для текущей ветки `main` теперь ведутся в [CHANGELOG.md](./CHANGELOG.md) в секции `Unreleased`, чтобы заметки про **выпущенный** `0.3.0` оставались строго привязанными к релизу.

Текущее примечание:

- выбор провайдера «на каждую линию перевода», идентичность перевода на основе слотов и вынос провайдеров перевода из legacy-слоя — это **последующие доработки** после `0.3.0` в ветке `main`; они описаны в [CHANGELOG.md](./CHANGELOG.md) в `Unreleased`, а не в дельте выпущенного `0.3.0`.
- текущие доработки в `main` также включают разделение UI вкладки Translation, более широкое покрытие i18n для дашборда, выравнивание хранилища вокруг `logs/` и `user-data/models/`, а также проверку обновлений через GitHub Releases (запрос в bootstrap-лаунчере + backend `POST /api/updates/check`); эти изменения тоже отражены в `Unreleased`.

Важно:

Ниже, в дополнение к дельте **выпущенного** `0.3.0`, перечислены небольшие UX hotfix после релиза, которые уже в `main` и воспринимаются как часть «текущего поведения 0.3.0» (без смены `PROJECT_VERSION`).

### Последующие доработки после 0.3.0: стабилизация рантайма P1 (в текущем `main`)
...
- логирование client-event не роняет backend при сбоях записи (нет обязательного `500`);
- экспериментальный browser worker `/google-asr-experimental` оформлен как поддерживаемый experimental-путь;
- неподдерживаемые эксперименты backend ASR убраны с активной desktop-поверхности; поддерживаемые пути — Parakeet и browser worker-ы.
...
### 4.1 Последующие UI hotfix (в текущем `main` после 0.3.0)
...
- Help / Помощь:
  ...
  - разделы покрывают обзор, распознавание/тюнинг, перевод, субтитры/стиль, OBS, инструменты/диагностику и режим desktop/remote;
...
- очистка выполняется через общий путь `destroy()` / `pagehide`;
...
Теперь работает в режиме с мягкой деградацией при ошибках записи:
...
### 10. Очистка поверхности ASR

Текущий проект после очистки поверхности после 0.3.0 поддерживает только:
...
- обработки сбоев session logger
- архитектуры frontend
- логирования дашборда
- remote-потока
- версионирования
- выбора провайдера ASR и очистки миграций
